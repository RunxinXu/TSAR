...
def main():
    ...
    # ======== load dglgraph ===========
    print('Load graph information...')
    train_dglgraph = torch.load(model_args.train_dglgraph_path)
    dev_dglgraph = torch.load(model_args.dev_dglgraph_path)
    test_dglgraph = torch.load(model_args.test_dglgraph_path)
    # ======================================
    ...
    if model_args.model_name_or_path.startswith('bert'):
        ...
        model = MyBertmodel.from_pretrained(
            ...
            revision=model_args.model_revision,
            use_auth_token=True if model_args.use_auth_token else None,
            gcn_layers=model_args.gcn_layers,
    ...
    def preprocess_function(example, idx, split):
        ...
        model_max_len = data_args.max_len
        if len(input_ids) > model_max_len - 1:
            input_ids = input_ids[:model_max_len - 1]
            belongingsnt = belongingsnt[:model_max_len-1]
        ...
        assert len(input_ids) == len(belongingsnt)
    ...
    train_dataset = datasets["train"].map(preprocess_function, batched=False, with_indices=True, load_from_cache_file=not data_args.overwrite_cache, fn_kwargs={'split': 'train'})
    eval_dataset = datasets["validation"].map(preprocess_function, batched=False, with_indices=True, load_from_cache_file=not data_args.overwrite_cache, fn_kwargs={'split': 'dev'})
    test_dataset = datasets["test"].map(preprocess_function, batched=False, with_indices=True, load_from_cache_file=not data_args.overwrite_cache, fn_kwargs={'split': 'test'})

    ###### graph.ndata['span'] word-level -> subword-level and combine multiple sentences
    def graph_wordspan2subwordspan(dglgraph, dataset):
        print('transform graph.ndata')
        for gs_idx, gs in tqdm(enumerate(dglgraph)):
            wordidx2subwordidx= dataset[gs_idx]['wordidx2subwordidx']
            snt2span = dataset[gs_idx]['snt2span']
            # data['span'] word-level -> subword-level
            assert len(gs) == len(snt2span)
            bias_L = 0
    ...
    def collator_fn(examples):
        ...
        for example in examples:
            ...
            belongingsnts.append(torch.LongTensor(example['belongingsnt']))

            # HACK to inject dglgraph
            if split == 'train':
                graphs.append(train_dglgraph[example['idx']])
            elif split == 'dev':
                graphs.append(dev_dglgraph[example['idx']])
            else:
                graphs.append(test_dglgraph[example['idx']])
        ...
        for span, span_len, label, subwords_span2snt, belongingsnt in zip(spans, span_lens, labels, subwords_span2snts, belongingsnts):
        ...
        belongingsnts = torch.nn.utils.rnn.pad_sequence(belongingsnts, batch_first=True, padding_value=0)
        ...
        subwords_snt2spans = torch.LongTensor(pad_subwords_snt2spans)
        subwords_span2snts = torch.LongTensor(pad_subwords_span2snts)
        trigger_snt_ids = torch.LongTensor(trigger_snt_ids)
        ...
        result = {
            ...
            'subwords_span2snts': subwords_span2snts,
